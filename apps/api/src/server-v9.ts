...
(express.application as any).listen=function(...args:any[]){
 if(!registered){
  ...
  this.use((error:any,_req:any,res:any,_next:any)=>{console.error('Core Ops Workflow v31 extension error',error);if(!res.headersSent)res.status(500).json({error:'internal server error'})});
 ...
 const last=args[args.length-1];if(typeof last==='function')args[args.length-1]=()=>{last();console.log('Core Ops Workflow API v31 service portfolio enabled')};
