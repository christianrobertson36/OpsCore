...
async function activateCentralLicence(key:string,actor:string){
 ...
 const response=await fetch(`${centralServerUrl}/api/client/check-in`,{
  ...
  body:JSON.stringify({licenceKey:key,installationId:lic.installationId,hostname:process.env.HOSTNAME||'',appVersion:'v15',usage:lic.usage||{},metadata:{product:'Core Ops Workflow',activation:'admin-ui'}})
 });
 ...
 await pool.query(`UPDATE licences SET licence_key=$1,licence_type=$2,plan_name=$3,status=$4,trial_ends_at=$5,expires_at=$6,grace_ends_at=$7,max_users=$8,max_sites=$9,max_assets=$10,notes=$11,updated_at=NOW() WHERE id=$12`,[
  key,remote.type||lic.licenceType,remote.planName||lic.planName,cachedStatus,remote.trialEndsAt||null,remote.expiresAt||null,remote.graceEndsAt||null,Math.max(1,Number(remote.maxUsers||lic.maxUsers)),Math.max(1,Number(remote.maxSites||lic.maxSites)),Math.max(1,Number(remote.maxAssets||lic.maxAssets)),'Activated and cached from Core Ops Licensing Portal',lic.id
 ]);
 ...
 (express.application as any)[method]=function(path:any,...handlers:any[]){
  ...
  if(typeof path==='string'){
   if(path==='/health'&&method==='get'){
    const wrapped=handlers.map((handler:any)=>(req:any,res:any,next:any)=>{const old=res.json.bind(res);res.json=(body:any)=>old({...body,app:'Core Ops Workflow API',version:'v15',licensing:'activation-and-sync',limits:'enforced',installation:'tracked',products:['OPSCORE','DCAM','SERVER_MANAGER']});return handler(req,res,next)});
    return original.call(this,path,...wrapped);
   }
   if(path==='/api/platform'&&method==='get'){
    const wrapped=handlers.map((handler:any)=>(req:any,res:any,next:any)=>{const old=res.json.bind(res);res.json=(body:any)=>old({...body,brand:'Core Ops Workflow',version:'v15',licensing:'activation-and-sync',limits:'enforced'});return handler(req,res,next)});
    return original.call(this,path,...wrapped);
   }
