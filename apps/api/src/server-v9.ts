...
import {ensureSlaV22,registerSlaV22} from './sla-v22.js';
import {ensureNotificationsV23,registerNotificationsV23} from './notifications-v23.js';
import {ensurePhase2V24,registerPhase2V24} from './phase2-v24.js';
import {ensurePhase3V25,registerPhase3V25} from './phase3-v25.js';
import {ensurePhase4V26,registerPhase4V26} from './phase4-v26.js';
import {ensurePhase5V27,registerPhase5V27} from './phase5-v27.js';
import {ensureHardeningV28,registerHardeningV28} from './hardening-v28.js';
import {ensurePhase7V29,registerPhase7V29} from './phase7-v29.js';
import {ensurePhase8V32,registerPhase8V32} from './phase8-v32.js';
import {ensurePhase9V34,registerPhase9V34} from './phase9-v34.js';
import {ensurePhase10V35,registerPhase10V35} from './phase10-v35.js';
import {ensurePhase11V37,registerPhase11V37} from './phase11-v37.js';
...
import {ensurePhase16V42,registerPhase16V42} from './phase16-v42.js';
import {ensurePhase17V43,registerPhase17V43} from './phase17-v43.js';
import {ensurePhase18V44,registerPhase18V44,startPhase18AutoSync} from './phase18-v44.js';
...
const API_VERSION='v44';
const WEB_VERSION='v57';
...
function productForPath(path:string){
 ...
 if(path.startsWith('/api/:recordType'))return 'OPSCORE' as const;
 ...
 if(path.startsWith('/api/incidents')||path.startsWith('/api/requests')||path.startsWith('/api/problems')||path.startsWith('/api/changes')||path.startsWith('/api/knowledge')||path.startsWith('/api/projects')||path.startsWith('/api/procurement')||path.startsWith('/api/monitoring')||path.startsWith('/api/sla')||path.startsWith('/api/notifications')||path.startsWith('/api/my-work')||path.startsWith('/api/approvals')||path.startsWith('/api/audit')||path.startsWith('/api/exports')||path.startsWith('/api/catalogue')||path.startsWith('/api/automations')||path.startsWith('/api/governance')||path.startsWith('/api/services')||path.startsWith('/api/portal')||path.startsWith('/api/feedback')||path.startsWith('/api/capacity')||path.startsWith('/api/service-desk')||path.startsWith('/api/cmdb')||path.startsWith('/api/user-experience'))return 'OPSCORE' as const;
 return null;
}
 ...
 (express.application as any)[method]=function(path:any,...handlers:any[]){
  ...
  if(typeof path==='string'){
   if(path==='/health'&&method==='get'){
    const wrapped=handlers.map((handler:any)=>(req:any,res:any,next:any)=>{const old=res.json.bind(res);res.json=(body:any)=>old({...body,app:'Core Ops Workflow API',version:API_VERSION,webVersion:WEB_VERSION,licensing:'activation-and-sync',limits:'enforced',installation:'tracked',enterpriseModules:'complete',dcamBridge:'ctec-shared-data-platform',monitoring:'http-tcp-foundation',sla:'policy-tracking-foundation',notifications:'in-app-alert-centre',products:['OPSCORE','DCAM','SERVER_MANAGER']});return handler(req,res,next)});
    return original.call(this,path,...wrapped);
   }
   if(path==='/api/platform'&&method==='get'){
    const wrapped=handlers.map((handler:any)=>(req:any,res:any,next:any)=>{const old=res.json.bind(res);res.json=(body:any)=>old({...body,brand:'Core Ops Workflow',version:API_VERSION,webVersion:WEB_VERSION,licensing:'activation-and-sync',limits:'enforced',enterpriseModules:'complete',dcamBridge:'ctec-shared-data-platform',monitoring:'http-tcp-foundation',sla:'policy-tracking-foundation',notifications:'in-app-alert-centre'});return handler(req,res,next)});
    return original.call(this,path,...wrapped);
   }
...
async function dcamRequest(path:string,options:RequestInit={}){const baseUrl=String(process.env.DCAM_API_URL||'').replace(/\/+$/,''),key=String(process.env.DCAM_COREOPS_INTEGRATION_KEY||'');if(!baseUrl||!key)throw new Error('DCAM integration is not configured');const controller=new AbortController(),timer=setTimeout(()=>controller.abort(),8000);try{const response=await fetch(`${baseUrl}${path}`,{...options,headers:{'X-DCAM-CoreOps-Key':key,...(options.headers||{})},signal:controller.signal}),body:any=await response.json().catch(()=>({}));if(!response.ok)throw new Error(body.error||body.detail||`DCAM HTTP ${response.status}`);return body}finally{clearTimeout(timer)}}
(express.application as any).listen=function(...args:any[]){
 if(!registered){
  ...
  registerSlaV22(this,pool,requireRoles as any);
  registerNotificationsV23(this,pool,requireRoles as any);
  registerPhase2V24(this,pool,requireRoles as any);
  registerPhase3V25(this,pool,requireRoles as any);
  registerPhase4V26(this,pool,requireRoles as any);
  registerPhase5V27(this,pool,requireRoles as any);
  registerHardeningV28(this,pool,requireRoles as any);
  registerPhase7V29(this,pool,requireRoles as any);
  registerPhase8V32(this,pool,requireRoles as any);
  registerPhase9V34(this,pool,requireRoles as any);
  registerPhase10V35(this,pool,requireRoles as any);
  registerPhase11V37(this,pool,requireRoles as any);
  ...
  registerPhase16V42(this,pool,requireRoles as any,dcamRequest);
  registerPhase17V43(this,pool,requireRoles as any,dcamRequest);
  registerPhase18V44(this,pool,requireRoles as any);
  ...
  this.use((error:any,_req:any,res:any,_next:any)=>{console.error('Core Ops Workflow v43 extension error',error);if(!res.headersSent)res.status(500).json({error:'internal server error'})});
  migrationsReady=Promise.all([licensing.ensureSchema(),ensureEnterpriseModulesV18(pool),ensureMonitoringV21(pool),ensureSlaV22(pool),ensureNotificationsV23(pool)]).then(()=>ensurePhase2V24(pool)).then(()=>ensurePhase3V25(pool)).then(()=>ensurePhase4V26(pool)).then(()=>ensurePhase5V27(pool)).then(()=>ensureHardeningV28(pool)).then(()=>ensurePhase7V29(pool)).then(()=>ensurePhase8V32(pool)).then(()=>ensurePhase9V34(pool)).then(()=>ensurePhase10V35(pool)).then(()=>ensurePhase11V37(pool)).then(()=>ensurePhase12V38(pool)).then(()=>ensurePhase13V39(pool)).then(()=>ensurePhase14V40(pool)).then(()=>ensurePhase15V41(pool)).then(()=>ensurePhase16V42(pool)).then(()=>ensurePhase17V43(pool)).then(()=>ensurePhase18V44(pool)).then(()=>startPhase18AutoSync(pool,dcamRequest));
  registered=true;
 }
 const last=args[args.length-1];if(typeof last==='function')args[args.length-1]=()=>{last();console.log('Core Ops Workflow API v44 automatic shared data sync enabled')};
 migrationsReady!.then(()=>originalListen.apply(this,args)).catch(error=>{console.error('Core Ops Workflow v43 initialisation failed',error);process.exit(1)});
 return this;
};
