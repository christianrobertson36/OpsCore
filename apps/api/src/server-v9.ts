...
async function activateCentralLicence(key:string,actor:string){
 ...
 await pool.query(`UPDATE licences SET licence_key=$1,licence_type=$2,plan_name=$3,status=$4,trial_ends_at=$5,expires_at=$6,grace_ends_at=$7,max_users=$8,max_sites=$9,max_assets=$10,notes=$11,updated_at=NOW() WHERE id=$12`,[key,remote.type||lic.licenceType,remote.planName||lic.planName,cachedStatus,remote.trialEndsAt||null,remote.expiresAt||null,remote.graceEndsAt||null,Math.max(1,Number(remote.maxUsers||lic.maxUsers)),Math.max(1,Number(remote.maxSites||lic.maxSites)),Math.max(1,Number(remote.maxAssets||lic.maxAssets)),'Activated and cached from Core Ops Licensing Portal',lic.id]);
