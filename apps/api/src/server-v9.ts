...
function productForPath(path:string){
 ...
 if(path.startsWith('/api/incidents')||path.startsWith('/api/requests')||path.startsWith('/api/problems')||path.startsWith('/api/changes')||path.startsWith('/api/knowledge')||path.startsWith('/api/projects')||path.startsWith('/api/procurement')||path.startsWith('/api/monitoring')||path.startsWith('/api/sla'))return 'OPSCORE' as const;
 return null;
}
...
(express.application as any).listen=function(...args:any[]){
 if(!registered){
  ...
  this.use((error:any,_req:any,res:any,_next:any)=>{console.error('Core Ops Workflow v22 extension error',error);if(!res.headersSent)res.status(500).json({error:'internal server error'})});
  Promise.all([licensing.ensureSchema(),ensureEnterpriseModulesV18(pool),ensureMonitoringV21(pool),ensureSlaV22(pool)]).catch(error=>console.error('Core Ops Workflow v22 initialisation failed',error));
  registered=true;
 }
 const last=args[args.length-1];if(typeof last==='function')args[args.length-1]=()=>{last();console.log('Core Ops Workflow API v22 SLA management enabled')};
