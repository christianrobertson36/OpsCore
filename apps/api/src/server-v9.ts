...
(express.application as any).listen=function(...args:any[]){
 if(!registered){
  ...
  migrationsReady=Promise.all([licensing.ensureSchema(),ensureEnterpriseModulesV18(pool),ensureMonitoringV21(pool),ensureSlaV22(pool),ensureNotificationsV23(pool)]).then(()=>ensurePhase2V24(pool)).then(()=>ensurePhase3V25(pool)).then(()=>ensurePhase4V26(pool)).then(()=>ensurePhase5V27(pool)).then(()=>ensureHardeningV28(pool)).then(()=>ensurePhase7V29(pool)).then(()=>ensurePhase8V32(pool)).then(()=>ensurePhase9V34(pool)).then(()=>ensurePhase10V35(pool)).then(()=>ensurePhase11V37(pool)).then(()=>ensurePhase12V38(pool)).then(()=>ensurePhase13V39(pool));
  registered=true;
 }
 const last=args[args.length-1];if(typeof last==='function')args[args.length-1]=()=>{last();console.log('Core Ops Workflow API v39 user experience controls enabled')};
