...
export function registerHardeningV28(app:Express,pool:any,requireRoles:(...roles:any[])=>any){
 app.get('/ready',async(_req:any,res:any)=>{const state=await readiness(pool);res.status(state.ok?200:503).json({...state,app:'Core Ops Workflow API',version:'v31',webVersion:'v39'})});
