...
export function registerMonitoringV21(app:Express,pool:any,requireRoles:RoleMiddleware){
  ...
  app.post('/api/monitoring/checks/:id/run',write,async(req:any,res:any,next:any)=>{
    try{
      ...
      await pool.query(`UPDATE monitoring_checks SET status=$1::varchar,last_response_ms=$2,last_message=$3,last_checked_at=NOW(),last_ok_at=CASE WHEN $1::text='Up' THEN NOW() ELSE last_ok_at END,updated_at=NOW() WHERE id=$4`,[status,result.responseMs,result.message,check.id]);
      ...
      if(status==='Down')await pool.query(`INSERT INTO notifications(title,message,severity,source_type,source_id,link_target,recipient_role,created_by) VALUES($1,$2,'Critical','Monitoring',$3,'Monitoring','Infrastructure','System') ON CONFLICT(source_type,source_id) WHERE source_type<>'' AND source_id<>'' DO UPDATE SET title=EXCLUDED.title,message=EXCLUDED.message,is_read=FALSE,read_at=NULL,updated_at=NOW()`,[`Monitoring check down: ${check.name}`,`${check.target} · ${result.message}`,String(check.id)]);
      else await pool.query(`UPDATE notifications SET severity='Success',title=$1,message=$2,is_read=FALSE,read_at=NULL,updated_at=NOW() WHERE source_type='Monitoring' AND source_id=$3`,[`Monitoring recovered: ${check.name}`,`${check.target} · ${result.message}`,String(check.id)]);
