...
const adminToken=process.env.LICENSING_ADMIN_TOKEN||'';
const clientSecret=process.env.LICENSING_CLIENT_SECRET||'';
...
function requireAdmin(req,res,next){if(!adminToken)return res.status(503).json({error:'licensing admin token not configured'});const token=String(req.headers.authorization||'').replace(/^Bearer\s+/i,'');if(token!==adminToken)return res.status(401).json({error:'unauthorised'});next()}
function requireClient(req,res,next){if(!clientSecret)return res.status(503).json({error:'licensing client secret not configured'});if(req.headers['x-coreops-client-secret']!==clientSecret)return res.status(401).json({error:'unauthorised installation'});next()}
...
app.get('/health',async(_req,res)=>{try{await pool.query('SELECT 1');res.json({ok:true,app:'Core Ops Licensing Portal',version:'v1',database:'connected'})}catch{res.status(503).json({ok:false,app:'Core Ops Licensing Portal',version:'v1',database:'unavailable'})}});
...
app.get('/api/admin/installations',requireAdmin,async(_req,res,next)=>{try{const r=await pool.query(`SELECT i.*,l.licence_key,c.name AS customer_name FROM central_installations i JOIN central_licences l ON l.id=i.licence_id JOIN licensing_customers c ON c.id=l.customer_id ORDER BY i.last_seen_at DESC`);res.json(r.rows)}catch(e){next(e)}});
...
ensureSchema().then(()=>app.listen(port,'0.0.0.0',()=>console.log(`Core Ops Licensing Portal v1 listening on ${port}`))).catch(e=>{console.error('Licensing portal startup failed',e);process.exit(1)});
