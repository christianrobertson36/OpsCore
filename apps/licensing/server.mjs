...
function versionInfo(row){
 const nested=row?.metadata?.metadata||{};
 const webVersion=String(nested.webVersion||'');
 const apiVersion=String(nested.apiVersion||row?.app_version||'');
 const display=webVersion?`Web ${webVersion} · API ${apiVersion||'—'}`:(apiVersion?`API ${apiVersion}`:'—');
 return {webVersion:webVersion||null,apiVersion:apiVersion||null,display};
}

app.get('/health',async(_req,res)=>{try{await pool.query('SELECT 1');res.json({ok:true,app:'Core Ops Licensing Portal',version:'v8',database:'connected',gui:'signed-persistent-session',versions:'web-and-api'})}catch{res.status(503).json({ok:false,app:'Core Ops Licensing Portal',version:'v8',database:'unavailable'})}});

app.post('/api/admin/login',(req,res)=>{if(!adminPassword||!sessionSigningKey)return res.status(503).json({error:'licensing admin session not configured'});const password=String(req.body?.password||'');if(!secureEqual(password,adminPassword))return res.status(401).json({error:'invalid password'});const now=Date.now();const token=signSession({role:'licensing-admin',createdAt:now,expiresAt:now+sessionTtlMs,nonce:crypto.randomBytes(12).toString('hex')});res.setHeader('Set-Cookie',`coreops_licensing_session=${encodeURIComponent(token)}; HttpOnly; SameSite=Strict; Path=/; Max-Age=${Math.floor(sessionTtlMs/1000)}`);res.json({ok:true,expiresIn:Math.floor(sessionTtlMs/1000),version:'v8',persistent:true})});
app.get('/api/admin/session',(req,res)=>{const s=sessionFrom(req);res.json({authenticated:Boolean(s),version:'v8',persistent:true,expiresAt:s?.expiresAt||null})});
...
app.get('/api/admin/installations',requireAdmin,async(_req,res,next)=>{try{const r=await pool.query(`SELECT i.*,l.licence_key,c.name AS customer_name,c.customer_code FROM central_installations i JOIN central_licences l ON l.id=i.licence_id JOIN licensing_customers c ON c.id=l.customer_id ORDER BY i.last_seen_at DESC`);res.json(r.rows.map(row=>{const v=versionInfo(row);return {...row,raw_app_version:row.app_version,web_version:v.webVersion,api_version:v.apiVersion,app_version:v.display}}))}catch(e){next(e)}});
...
ensureSchema().then(()=>app.listen(port,'0.0.0.0',()=>console.log(`Core Ops Licensing Portal v8 listening on ${port}`))).catch(e=>{console.error('Licensing portal startup failed',e);process.exit(1)});
