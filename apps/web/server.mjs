...
app.get('/health', (_req, res) => res.json({ ok: true, app: 'OpsCore Web', version: 'v6', apiProxy: apiUrl.origin }));
app.use('/api', proxyToApi);
app.use('/auth', proxyToApi);
app.use(express.static(dist));
app.use((_req, res) => res.sendFile(path.join(dist, 'index.html')));

app.listen(port, '0.0.0.0', () => console.log(`OpsCore Web v6 listening on ${port}; API proxy ${apiUrl.origin}`));