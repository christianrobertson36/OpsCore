import './branding-v22.css';

document.documentElement.classList.add('coreopsV22');
...
function setupCompactMenu(nav){
  if(!nav||nav.dataset.compactMenu==='22')return;
  nav.dataset.compactMenu='22';
...
function bootNavigation(){
  ...
  if(nav){setupCompactMenu(nav);return true}
  return false;
}
...
if(!bootNavigation()){
  ...
  if(root){
    const observer=new MutationObserver(()=>{if(bootNavigation())observer.disconnect()});
...
window.addEventListener('coreops:language-changed',()=>{
  const nav=document.querySelector('.app aside nav');
  if(!nav)return;
  nav.dataset.compactMenu='';
  setupCompactMenu(nav);
});
